...
async function showQualityMenu(sock, from, meta, sender, ownerRefs = []) {
  ...
  if (ui.isButtonModeOn({ sender, chatJid: from })) {
    return await ui.sendQualityMenu(sock, from, meta, { send: { quoted: meta.msg || null } }, { sender, ownerRefs, chatJid: from });
  }
...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
    ...
    if (/^\d+$/.test(lower)) {
      ...
      const hasMenuMarker = quotedText.includes('Reply') || quotedText.includes('PRO PANEL') || quotedText.includes('AI CENTER') || quotedText.includes(roleMenu.MENU_MARKER);

      if (hasMenuMarker || isDirectPm) {
        // 1. Video Quality Selection
        const videoKW = themeMgr.getAllKeywords("video_ready");
        const isVideoQuoted = videoKW.some(kw => quotedText.includes(kw));
        const qEntry = qualitySelection.get(sender);
        if (qEntry && (isVideoQuoted || isDirectPm) && num >= 1 && num <= 4) {
        ...
        const isMusicQuoted = musicKW.some(kw => quotedText.includes(kw));
        const pEntry = playSelection.get(sender);
        if (pEntry && (isMusicQuoted || isDirectPm) && num >= 1 && num <= 4) {
          sendReact(sock, from, msg, "⏳");
        ...
        const isActionQuoted = actionKW.some(kw => quotedText.includes(kw));
        const sEntry = lastSearch.get(sender);
        if (sEntry && (isActionQuoted || isDirectPm) && num >= 1 && num <= sEntry.results.length) {
          sendReact(sock, from, msg, "🎬");
