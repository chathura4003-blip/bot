...
const { sendReact, presenceUpdate, truncate, isOwner, withOwnerContext, downloadMediaMessage, cleanJid, canonicalSender, getRealMessage, extractQuotedContext } = require("./utils");
...
const processedMsgIds = new LRUCache({
  max: 5000,
  ttl: 120000, // 2 minutes retention
});
...
async function handleCommand(sock, msg, from, text, disabledModules = [], context = {}) {
  ...
  try {
    ...
    const cmdText = (realMsg?.conversation ||
    ...
    // commands continue to flow through the normal pipeline below.
    {
      ...
      if (earlySel) {
        if (msg.key?.id) processedMsgIds.set(msg.key.id, true);
    ...
    // as a button tap, a list selection, or a native-flow response.
    ...
    if (/^\d+$/.test(lower)) {
      ...
      const quotedCtx = quoted.contextInfo;
      const quotedMsg = quoted.quotedMsg;
