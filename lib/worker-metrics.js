...
class WorkerMetrics {
  ...
  updateTaskProgress(taskId, { percent, speed, eta, sizeBytes, totalBytes, stage, status, formattedSize, uploadedBytes }) {
    ...
    if (formattedSize !== undefined) task.formattedSize = formattedSize;
    else if (sizeBytes && !task.formattedSize) task.formattedSize = this.formatBytes(sizeBytes);
    if (uploadedBytes !== undefined) task.uploadedBytes = uploadedBytes;
  }
