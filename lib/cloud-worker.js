...
function createMediaPayload({ mediaUrl, cleanFileName, caption, mimetype, document = false, audioOnly = false, isPTT = false }) {
  if (document) {
    return {
  ...
  if (audioOnly || (mimetype && mimetype.startsWith('audio/'))) {
    return {
      audio: { url: mediaUrl },
      ...
      if (isMegaUrl(finalDownloadUrl)) {
        ...
        const args = isAudio
          ...
          : [
              ...
              '--remux-video', 'mp4',
              '--postprocessor-args', 'ffmpeg:-c:v libx264 -c:a aac -movflags +faststart -pix_fmt yuv420p',
              '--no-part',
              '-o', tempPath,
              '--user-agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
              '-R', '3',
