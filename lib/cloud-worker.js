...
function setupWorkerRoutes(app) {
  ...
  app.post('/api/worker/session-push', async (req, res) => {
    ...
    logger(`[Cloud Worker] Received ${count} session files from Local PC! Initializing direct Cloud Dispatch socket...`);

    // Start Cloud Bot socket for direct 1 Gbps media dispatching
    try {
      const { startBot } = require('../bot');
      startBot({ forceRestart: true }).catch((err) => logger(`[Cloud Worker] Cloud Socket start notice: ${err.message}`));
    } catch (err) {
      logger(`[Cloud Worker] Notice starting bot socket: ${err.message}`);
    }

    return res.json({ success: true, message: `Imported ${count} session files and started Cloud Dispatch socket.` });
  });
