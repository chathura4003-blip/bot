...
function setupWorkerRoutes(app) {
  ...
  // Dedicated Cloud-to-Drive Transfer Endpoints (0 MB PC/Phone Data Consumption)
  const activeDriveTasks = new Map();

  app.get('/api/worker/drive-status/:taskId', (req, res) => {
    const task = activeDriveTasks.get(req.params.taskId);
    if (!task) return res.status(404).json({ error: 'Task not found' });
    res.json(task);
  });

  app.post('/api/worker/drive-transfer', async (req, res) => {
    const { taskId = `gdrive_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`, sourceUrl, title, folderId, accessToken, refreshToken, clientId, clientSecret, type = 'media' } = req.body;

    if (!sourceUrl || (!accessToken && !refreshToken)) {
      return res.status(400).json({ error: 'sourceUrl and Google Drive tokens are required' });
    }

    const taskState = {
      taskId,
      title: title || 'Cloud File',
      status: 'downloading',
      percent: 0,
      speedMBps: '0.00',
      uploadedMB: '0.0',
    ...
    activeDriveTasks.set(taskId, taskState);
    res.json({ success: true, taskId, message: 'Cloud-to-Drive Transfer started on Railway Cloud Worker' });

    // Execute in background on Railway 1 Gbps line
    (async () => {
      try {
        let token = accessToken;
        if (!token && refreshToken) {
          const tRes = await axios.post('https://oauth2.googleapis.com/token', {
            client_id: clientId,
            client_secret: clientSecret,
            refresh_token: refreshToken,
            grant_type: 'refresh_token'
          });
          token = tRes.data.access_token;
        }

        let targetUrl = sourceUrl;
        let cleanFileName = (title || 'cloud_file').replace(/[/\\?%*:|"<>]/g, '_');
        if (!cleanFileName.includes('.')) cleanFileName += '.mp4';

        const tempPath = path.join(config.DOWNLOAD_DIR, `gdrive_worker_${Date.now()}_${cleanFileName}`);
        taskState.status = 'downloading';

        const isDirect = /pixeldrain\.com\/api\/file|workers\.dev|ddl\.sinhalasub|usersdrive|\.(mp4|mkv|zip|rar|mp3|webm|m4a)(\?.*)?$/i.test(targetUrl);

        if (isDirect) {
          const { downloadParallel } = require('./parallel-downloader');
          await downloadParallel({
            url: targetUrl,
            destPath: tempPath,
            concurrency: 8,
            onProgress: (p) => {
              taskState.percent = Math.round(p.percent / 2);
              taskState.speedMBps = p.speedMBps || '45.00';
              taskState.uploadedMB = (p.downloadedBytes / (1024 * 1024)).toFixed(1);
              taskState.totalMB = (p.totalBytes / (1024 * 1024)).toFixed(1);
              taskState.etaSec = p.etaSec || 0;
            }
          });
        } else {
          const { ensureYtdlp, getBinPath, getCommonArgs, FFMPEG_PATH } = require('./ytdlp-manager');
          const { spawn } = require('child_process');
          await ensureYtdlp();
          
          await new Promise((resolve, reject) => {
            const child = spawn(getBinPath(), [
              ...getCommonArgs(),
        ...
        const metadata = { name: cleanFileName, mimeType: 'video/mp4' };
        if (targetFolder !== 'root') metadata.parents = [targetFolder];
        ...
        fileStream.on('data', (chunk) => {
          ...
          const elapsed = (Date.now() - startUploadTime) / 1000 || 1;
          const speed = (uploaded / elapsed) / (1024 * 1024);
          taskState.percent = 50 + Math.round((uploaded / fileSize) * 50);
          taskState.speedMBps = speed.toFixed(2);
          taskState.uploadedMB = (uploaded / (1024 * 1024)).toFixed(1);
          taskState.totalMB = (fileSize / (1024 * 1024)).toFixed(1);
          taskState.etaSec = Math.round((fileSize - uploaded) / (speed * 1024 * 1024));
        });

        const driveUploadRes = await axios.put(uploadUrl, fileStream, {
          headers: {
        ...
        const fileData = driveUploadRes.data;
        try {
          await axios.post(
  ...
  app.post('/api/worker/upload-media', async (req, res) => {
    let taskId = req.body.taskId || `${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    try {
      ...
      logger(`[Cloud Worker] Starting offloaded ${isAudio ? 'audio' : 'video'} task for ${targetJid}: ${fileName || 'media'} (TaskId: ${taskId})`);
      ...
      if (isMegaUrl(finalDownloadUrl)) {
        ...
        logger(`[Cloud Worker] 🚀 Starting 8x Parallel Segmented Stream from: ${finalDownloadUrl}`);
        const { downloadParallel } = require('./parallel-downloader');
        await downloadParallel({
          url: finalDownloadUrl,
          destPath: tempPath,
          concurrency: 8,
          onProgress: ({ downloadedBytes, totalBytes, percent, speedMBps, numSegments }) => {
            const formattedSize = totalBytes > 0 ? workerMetrics.formatBytes(totalBytes) : workerMetrics.formatBytes(downloadedBytes);
            const speedStr = speedMBps ? `${speedMBps} MB/s` : '1 Gbps Pipe';
            lastKnownSpeed = speedStr;
            workerMetrics.updateTaskProgress(taskId, {
              stage: 'downloading',
              status: `Downloading to Cloud (${numSegments || 8}x Parallel)`,
              percent,
              speed: speedStr,
 ...
 */
async function offloadMediaToWorker({ targetJid, downloadUrl, fileName, caption, mimetype, document = true, audioOnly = false, isPTT = false, quoted = null, onProgress = null }) {
  const workerUrl = (config.CLOUD_WORKER_URL || process.env.CLOUD_WORKER_URL || '').trim();
  if (!workerUrl) {
  ...
  const statsEndpoint = `${workerUrl.replace(/\/$/, '')}/api/worker/stats`;
  const appState = require('../state');
  ...
  if (typeof onProgress === 'function') {
    progressPoller = setInterval(async () => {
      try {
        const statsRes = await axios.get(statsEndpoint, { timeout: 3000 });
        if (statsRes.data && Array.isArray(statsRes.data.activeTasks)) {
          const myTask = statsRes.data.activeTasks.find((t) => t.taskId === taskId);
          if (myTask) {
            onProgress(myTask);
          }
        }
  ...
  try {
    logger(`[Local Bot] Offloading media upload to Cloud Worker: ${endpoint} (TaskId: ${taskId})`);
    const res = await axios.post(
      endpoint,
      { taskId, targetJid, downloadUrl, fileName, caption, mimetype, document, audioOnly, isPTT, mediaConn },
      {
        headers: {
