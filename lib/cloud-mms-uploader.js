...
const { httpsAgent } = require('./parallel-downloader');
...
async function uploadDirectToWhatsAppMMS({
  ...
  const formattedSize = workerMetrics.formatBytes(fileLength);
  if (taskId) {
    workerMetrics.updateTaskProgress(taskId, {
      stage: 'encrypting',
      status: 'Encrypting for WhatsApp',
  ...
  logger(`[Cloud MMS] 🔒 Encrypting ${formattedSize} (${mediaType}) on Railway Datacenter...`);
  ...
  for (let i = 0; i < hosts.length; i++) {
    ...
    let uploadedBytes = 0;
    const bodyStream = fs.createReadStream(encPath, { highWaterMark: 2 * 1024 * 1024 });
