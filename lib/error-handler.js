...
async function retryWithBackoff(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      lastErr = err;
      logger(
        `[${context}] Attempt ${attempt}/${maxAttempts} failed: ${err?.message}`,
      );
...
function handleAPIError(err, context = "API") {
  ...
  if (code === 410 || msg.includes("410") || msg.includes("Gone")) {
    return { message: "⚠️ මෙම වීඩියෝව අදාළ වෙබ් අඩවියෙන් ඉවත් කර ඇත (Video Removed / Expired 410). කරුණාකර වෙනත් වීඩියෝවක් තෝරන්න." };
  }
  if (code === 404) {
