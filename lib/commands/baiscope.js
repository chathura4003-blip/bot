...
//   1. Soft ceiling: process memory headroom (dynamic, ~1500 MB max).
//   2. User override: process.env.BAISCOPE_MAX_UPLOAD_MB.
//   3. Hard ceiling: WhatsApp's own document limit (1900 MB).
const WHATSAPP_DOC_LIMIT_MB = 1900;
...
function getEffectiveMaxUploadBytes() {
  ...
  if (Number.isFinite(envOverride) && envOverride > 0) {
    return Math.min(envOverride, WHATSAPP_DOC_LIMIT_MB) * 1024 * 1024;
  }
  return Math.min(1500 * 1024 * 1024, hardLimit);
}
...
function getDirectPixeldrainUrl(url) {
  if (!url) return null;
  // Pixeldrain has two URL forms:
  //   /u/<fileId>  → a single uploaded file (direct video).
  //   /l/<listId>  → a list/album that can contain many files.
  // /api/file/<id> only works for file IDs; passing a list ID returns a
  // small "file_not_found" JSON page, which used to get uploaded as the
  // "movie" and arrive in WhatsApp as a few-KB document.
  const fileMatch = url.match(/pixeldrain\.com\/u\/(\w+)/);
  if (fileMatch) return `https://pixeldrain.com/api/file/${fileMatch[1]}?download`;
  return url;
}

async function resolvePixeldrainListUrl(listUrl) {
  // Resolve /l/<id> list URLs by fetching list metadata and picking the
  // largest file (typically the movie itself, not subtitles/extras).
  try {
    const m = listUrl.match(/pixeldrain\.com\/l\/(\w+)/);
    if (!m) return null;
...
async function probeDirectVideoUrl(url) {
  // Returns { ok, contentLength, contentType, finalUrl } after following
  // redirects. We try a HEAD first (cheap), then fall back to a ranged
  // GET that immediately aborts the body — some hosts (pixeldrain,
  // workers.dev) reject HEAD or don't return content-type on it.
  const headers = {
...
async function baiscopeSearch(query) {
  ...
  try {
    ...
    let searchUrl = `https://baiscopes.lk/?s=${encodeURIComponent(query)}&post_type=movies`;
    const cleanQ = (query || "").trim().toLowerCase();
    if (!cleanQ || cleanQ === "new" || cleanQ === "latest" || cleanQ === "all") {
      searchUrl = `https://baiscopes.lk/movies/`;
    }

    await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => { });
    await page.waitForSelector("article, .items .item, .movies .movie", { timeout: 8000 }).catch(() => { });

    let results = await page.evaluate(() => {
      let articles = Array.from(document.querySelectorAll("article, .items .item, .movies .item"));
      const found = [];
      articles.slice(0, 10).forEach((article, index) => {
        const titleLink = article.querySelector(".title a, .data h3 a, h3 a, a[href*='/movies/']");
        ...
        found.push({ id: index + 1, title: title.replace(/\s+/g, ' '), url: titleLink.href, language: "Sinhala Sub", quality: "HD", thumbnail });
      });
      return found;
    });

    if (!results.length && cleanQ && cleanQ !== "new" && cleanQ !== "latest") {
      const fallbackUrl = `https://baiscopes.lk/?s=${encodeURIComponent(query)}`;
      await page.goto(fallbackUrl, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => { });
      results = await page.evaluate(() => {
        let articles = Array.from(document.querySelectorAll("article, .items .item, .result-item"));
        const found = [];
        articles.slice(0, 10).forEach((article, index) => {
          const titleLink = article.querySelector(".title a, .data h3 a, h3 a, a[href*='/movies/']");
          if (!titleLink || !titleLink.href) return;
...
async function baiscopeResolveLink(browser, linkData, onWait = null) {
  ...
  try {
    ...
    // 1. Instant check for direct download button or zluFinalLink
    for (let i = 0; i < 3; i++) {
      ...
      const instantLink = await subPage.evaluate(() => {
        ...
        const target = document.querySelector("a#link, a.btn[href*='workers.dev'], a[href*='workers.dev'], a[href*='pixeldrain'], a[href*='drive.google'], a[href*='usersdrive'], .wait-done a");
        return (target && target.href && !target.href.includes('#') && !target.href.includes('javascript')) ? target.href : null;
      }).catch(() => null);

      if (instantLink && (instantLink.includes('workers.dev') || instantLink.includes('pixeldrain') || instantLink.includes('google') || instantLink.includes('usersdrive') || /\.(mp4|mkv|zip|rar)$/i.test(instantLink))) {
    ...
    if (!directDetected) {
      ...
      subPage.on('request', (req) => {
        ...
        const isVideoFile = (url.includes('.mp4') || url.includes('.mkv') || url.includes('workers.dev') || (url.includes('/api/file/') && !url.includes('/thumbnail') && !url.includes('/gallery'))) && !url.includes('cdn-cgi');
      ...
      targetListener = async (target) => {
        ...
        if (tUrl && !directDetected) {
          const isDL = (tUrl.includes('pixeldrain.com') || tUrl.includes('drive.google') || tUrl.includes('workers.dev') || tUrl.includes('drive.usercontent') || /\.(mp4|mkv|zip|rar|mp3)$/i.test(tUrl));
          ...
          if (newPage) {
            ...
            newPage.on('response', response => {
              ...
              const isDL = (contentType.includes('video/') || rUrl.includes('.mp4') || rUrl.includes('.mkv') ||
                rUrl.includes('pixeldrain.com') || rUrl.includes('drive.google') || rUrl.includes('workers.dev') || rUrl.includes('drive.usercontent'));
      ...
      for (let j = 0; j < 3; j++) {
        ...
        if (finalExtract && (finalExtract.includes('workers.dev') || finalExtract.includes('pixeldrain') || finalExtract.includes('google') || finalExtract.includes('usersdrive'))) {
...
async function baiscopeGetDetails(movieUrl, sender) {
  ...
  // Disable console logs to keep terminal clean
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      // Target the specific Baiscopes download table, prioritizing #dt_contenedor area
      const targetTable = document.querySelector("#dt_contenedor #download.sbox table, #download.sbox .links_table table, #dt_contenedor table");
      const targetRows = targetTable ? targetTable.querySelectorAll("tbody tr, tr") : document.querySelectorAll("#download .links_table tr");

      if (targetRows.length > 0) {
        targetRows.forEach((row, index) => {
          try {
            const a = row.querySelector("a[href*='/links'], a[href*='goto'], a[href*='pixeldrain']");
            const href = a?.href || "";
            ...
            let qualityText = cells[1]?.textContent.trim() || row.querySelector(".quality")?.textContent.trim() || "";
            let sizeText = cells[2]?.textContent.trim() || row.querySelector(".size")?.textContent.trim() || "";
            const fullText = row.innerText || "";

            if (!qualityText || !/\d/.test(qualityText)) {
              const qMatch = fullText.match(/(1080p|720p|480p|360p|FHD|HD|SD)/i);
              qualityText = qMatch ? qMatch[0] : "720p";
            }
            if (!sizeText || sizeText === "N/A") {
              const sMatch = fullText.match(/\d+(\.\d+)?\s*(GB|MB)/i);
              sizeText = sMatch ? sMatch[0] : "N/A";
            }

            const quality = qualityText.match(/\d{3,4}p/i)?.[0] || (qualityText.toLowerCase().includes("hd") ? "720p" : "480p");
            let source = "Link";
            if (href.includes("pixeldrain")) source = "Pixeldrain";
            else if (href.includes("drive.google")) source = "G-Drive";
            else if (href.includes("usersdrive")) source = "UsersDrive";

            // User's specific priority logic
            let p = 99;
            if (index === 2 || index === 3) p = 1;
            else if (index === 0) p = 2;
            else if (index === 1) p = 3;
            else p = 4 + index;

            links.push({ pageLink: href, quality, size: sizeText, source, link: href, priority: p });
          } catch (e) { }
        });
      }
...
module.exports = {
      ...
      if (mode === "dlmovie") {
        ...
        if (directUrl) {
          ...
          if (probedBytes > 0 && probedBytes > maxUploadBytes) {
            ...
            console.error(`[Baiscope] Skipping upload: file ${sizeMB} MB exceeds host upload cap (${capMB} MB)`);
            const limitText =
              "╭━━━━━〔 ᴄʜᴀᴛʜᴜ-ᴍᴅ ᴠ4 〕━━━━━\n" +
              `┃ 🎬 𝐌𝐨𝐯𝐢𝐞 : ${truncate(cache.metadata.title, 20)}\n` +
              "┃ 🚫 *FILE TOO LARGE FOR THIS HOST!*\n" +
              `┃ 📦 File Size : ${sizeMB} MB\n` +
              `┃ ⚠️ Host Cap  : ${capMB} MB\n` +
              "┃ වෙනත් quality අඩුවක් select කරන්න\n" +
              "┃ හෝ වෙනත් lower-RAM plan එකකට upgrade කරන්න.\n" +
              "╰━━━━━━━━━━━━━━━━━━━━━━\n" +
              `🔗 *Manual Link:* ${actualLink}`;
          ...
          // has room. This is the single biggest OOM mitigation on small
          // hosts.
  ...
  async onMessage({ sock, from, msg, text, sender, isOwner }) {
    ...
    if (global.baiscopeSearchCache && global.baiscopeSearchCache[sender]) {
      ...
      const isQuoted = quotedId && quotedId === cache.menuId;
      const isRecent = (now - cache.timestamp) < timeout;
      if ((isQuoted || (isDirectPm && isRecent)) && num >= 1 && num <= cache.results.length) {
