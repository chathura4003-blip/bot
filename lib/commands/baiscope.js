  ...
  try {
    ...
    if (res.status === 200 && res.data) {
      ...
      if (title) {
        ...
        const links = [];
        const seen = new Set();
        const targetRows = $("#dt_contenedor #download.sbox table tr, #download.sbox .links_table table tr, #dt_contenedor table tr, #download .links_table tr, table tr");

        targetRows.each((index, row) => {
          const a = $(row).find("a[href*='/links'], a[href*='goto'], a[href*='pixeldrain']");
          ...
          let qualityText = $(cells[1]).text().trim() || $(row).find(".quality").text().trim() || "";
          let sizeText = $(cells[2]).text().trim() || $(row).find(".size").text().trim() || "";
