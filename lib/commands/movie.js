  ...
  try {
    ...
    while (Date.now() - pollStart < 4000 && !directDetected) {
      const instantLink = await subPage.evaluate(() => {
        ...
        const target = document.querySelector("a#link, a.btn[href*='workers.dev'], a[href*='pixeldrain'], a[href*='ddl.sinhalasub'], a[href*='drive.google'], a[href*='mega.nz'], .link-redirector .wrapper .wait-done a, .wait-done a");
        return target && target.href ? target.href : null;
      }).catch(() => null);

      if (instantLink && (instantLink.includes('pixeldrain') || instantLink.includes('ddl.sinhalasub') || instantLink.includes('drive.google') || instantLink.includes('workers.dev') || instantLink.includes('mega.nz') || instantLink.includes('mega.co') || /\.(mp4|mkv|zip|rar)$/i.test(instantLink))) {
    ...
    if (!directDetected) {
      ...
      while (Date.now() - clickPollStart < 5000 && !directDetected) {
        const finalExtract = await subPage.evaluate(() => {
          ...
          if (match && match[1]) return match[1];
          const target = document.querySelector("a#link, a[href*='pixeldrain'], a[href*='ddl.sinhalasub'], a[href*='drive.google'], a[href*='mega.nz'], a.wait-done a");
          return target && target.href ? target.href : null;
        }).catch(() => null);

        if (finalExtract && (finalExtract.includes('pixeldrain') || finalExtract.includes('ddl.sinhalasub') || finalExtract.includes('drive.google') || finalExtract.includes('workers.dev') || finalExtract.includes('mega.nz') || finalExtract.includes('mega.co'))) {
            ...
            if (CLOUD_WORKER_URL) {
              ...
              const offloadRes = await offloadMediaToWorker({
                ...
                onProgress: async (task) => {
                  const now = Date.now();
                  if (now - lastMsgUpdate < 1500) return;
                  lastMsgUpdate = now;

                  const p = Math.min(100, Math.max(0, Math.round(task.percent || 0)));
                  const bar = progressBar(p, 100, 10);
                  const sizeText = task.formattedSize || (task.sizeBytes ? `${(task.sizeBytes / (1024 * 1024)).toFixed(1)} MB` : 'Calculating...');

                  let stageText = "📥 Cloud Server එකට 1 Gbps Line එකෙන් බාගත වේ...";
                  if (task.stage === 'downloaded') {
                    stageText = "✅ Cloud Download සම්පූර්ණයි. WhatsApp වෙත සූදානම් වේ...";
                  } else if (task.stage === 'encrypting') {
                    stageText = "⚙️ WhatsApp වෙත Encrypt කරමින් පවතී...";
                  } else if (task.stage === 'uploading_mms' || task.stage === 'uploading') {
                    stageText = `📤 WhatsApp වෙත Cloud එකෙන් Upload වේ... (~${p}%)`;
                  }
              ...
              console.log('[Movie] Cloud worker offload fallback to local:', offloadRes.error || offloadRes.reason);
            }
