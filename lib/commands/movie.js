...
async function sinhalasubGetDetails(movieUrl) {
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      // Strictly following user's requested selectors for DLServer, Pixeldrain and Redirector
      const selectors = [
        ".link-redirector .wait-done",
        ".link-redirector .wait-done tr",
        ".link-DLServer-01.active tr",
        ".link-pixeldrain tr",
        ".link-DLServer-02 tr"
      ];
      ...
      selectors.forEach(sel => {
        document.querySelectorAll(sel).forEach(row => {
          try {
            ...
            // Strictly allow ONLY sinhalasub.lk/links/ links
            if (!href.includes('sinhalasub.lk/links/')) return;
            ...
            const quality = qualityText.match(/\d{3,4}p/i)?.[0] || "720p";
            
            // Skip 1080p links as requested
            if (quality === "1080p") return;

            // Extract Size
            let sizeText = cells[2]?.textContent.trim() || row.querySelector(".size")?.textContent.trim() || "N/A";
            ...
            let source = "Server";
            if (sel.includes("DLServer-01")) source = "Server 1";
            else if (sel.includes("DLServer-02")) source = "Server 2";
            else if (sel.includes("pixeldrain")) source = "Pixeldrain";

            links.push({ pageLink: href, quality, size: sizeText, source, link: href });
          } catch (e) { }
        });
      });
...
module.exports = {
      ...
      if (mode === "dlmovie") {
        ...
        if (directUrl) {
          ...
          try {
            ...
            // Enforce WhatsApp 1900MB limit before downloading large file
            if (totalBytes > WHATSAPP_DOC_LIMIT_BYTES) {
              try { response.data.destroy(); } catch (_) { }
              const sizeMB = (totalBytes / (1024 * 1024)).toFixed(2);
              console.error(`[Movie] Skipping upload: file ${sizeMB} MB exceeds WhatsApp limit (${WHATSAPP_DOC_LIMIT_MB} MB)`);
              const limitText =
                "╭━━━━━〔 ᴄʜᴀᴛʜᴜ-ᴍᴅ ᴠ4 〕━━━━━\n" +
                `┃ 🎬 𝐌𝐨𝐯𝐢𝐞 : ${truncate(cache.movie.metadata.title, 20)}\n` +
                "┃ 🚫 *WHATSAPP LIMIT EXCEEDED!*\n" +
                `┃ 📦 File Size : ${sizeMB} MB\n` +
                `┃ ⚠️ Max Allowed : ${WHATSAPP_DOC_LIMIT_MB} MB\n` +
                "╰━━━━━━━━━━━━━━━━━━━━━━\n" +
  ...
  async onMessage({ sock, from, msg, text, sender, isOwner }) {
    ...
    if (global.sinhalasubQualityCache && global.sinhalasubQualityCache[sender]) {
      ...
      if ((isQuoted || (isDirectPm && isRecent)) && num >= 1 && num <= cache.movie.downloadLinks.length) {
